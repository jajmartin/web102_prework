...
function showAllGames() {
    ...
    let allGames = GAMES_JSON.filter( (game) => {return game;});
...
let numUnfunded = GAMES_JSON.filter( (game) => {return game.pledged < game.goal;});
numUnfunded = numUnfunded.length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = 
    `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames.toLocaleString('en-US')} games. 
    Currently, ${numUnfunded.toLocaleString('en-US')} ${numUnfunded > 1 ? "games remain" : "game remains"} unfunded. We need 
